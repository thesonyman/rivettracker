...
if (isset($_POST["download"]))
{
	...
	print "\$GLOBALS['exclusiveannounce'] = " . htmlspecialchars($_POST["exclusiveannounce"]) . ";\n";
...
echo <<<HTML
<style>table {width: 45%; margin: auto;} th {background: transparent; border: none; align: center;} td.a {width: 25%; text-align: center;} td.c {width: 50px; margin: auto;}</style>
HTML;
...
echo ("<br><br>");
		if (!version_compare(PHP_VERSION, '4.4.9', '<=')) echo("<center><font face=\"Verdana\">Fully supported. You may continue.</font></center>");
		if (version_compare(PHP_VERSION, '4.4.9', '<=')) echo "<center><font face=\"Verdana\">Not fully supported, but you may try.</font></center>";
		if ((!$_GET['mysql_version'])) die("<center><font face=\"Verdana\">Cannot connect to database. You may not continue.</font></center>");
		if ((version_compare(PHP_VERSION, '4.4.9', '<=')) && (!$_GET['mysql_version'])) die("<center><font face=\"Verdana\">Not fully supported, also cannot connect to database. You may not continue.</font></center>");
		?>
		<br>
		<br>
		<center>
	...
	$prefix = $_POST["prefix"];
	$makenamemap= 'CREATE TABLE ' . $prefix . 'namemap (info_hash char(40) NOT NULL default "", title varchar(250) NOT NULL default "", filename varchar(250) NOT NULL default "", url varchar(250) NOT NULL default "", size bigint(20) unsigned NOT NULL, pubDate varchar(50) NOT NULL default "", PRIMARY KEY(info_hash)) ENGINE = innodb'; 	
	...
	if (isset($_POST["domakeaccount"]) || isset($_POST["maketables"]))
	{
		...
		<tr><td><b>Have exclusive torrents.  If checked, the tracker will only accept torrents assigned to this tracker.</b></td>
		<td><input type="checkbox" name="exclusiveannounce"></td></tr>
		...
		<tr><td><b><span class="notice">* </span>Date format of the torrent publication date. It shows on statistics.php. If you change this setting, you will have to change it for every other existing torrent!</b></td>
		<td>
	...
	if (isset($_POST["config"]))
	{
		...
		if (!is_writable("./"))
		{
			...
			<input type="hidden" name="exclusiveannounce" value="<?php if ($_POST['exclusiveannounce'] == 'on') echo 'true'; else echo 'false';?>">
			<input type="hidden" name="scrape" value="<?php if ($_POST['scrape'] == 'on') echo 'true'; else echo 'false';?>">
			<input type="hidden" name="customtitle" value="<?php if ($_POST['customtitle'] == 'on') echo 'true'; else echo 'false';?>">			
